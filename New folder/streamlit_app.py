...
if st.button("Generate 3-Statement Outputs", type="primary"):
    ...
    # Persist output bytes in session so the download button remains after reruns
    st.session_state['last_excel_bytes'] = out_bytes.getvalue()
    st.session_state['last_financial_data'] = financial_data


    st.success("Generated Excel output.")
    # Always show download + on-page preview
    st.download_button(
    ...
    # Show output on the website (table view) using the same computed financial_data
    yrs = sorted(st.session_state['last_financial_data'].keys())
    df_preview = pd.DataFrame(st.session_state['last_financial_data']).fillna(0.0)
    df_preview = df_preview[yrs] if yrs else df_preview
    df_preview = df_preview / float(st.session_state['unit_scale'])
    st.subheader("3-Statement Output Preview")
    st.caption("Values are shown in the selected unit scale; download Excel for the full formatted model.")
